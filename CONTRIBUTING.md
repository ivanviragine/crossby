...
2. Create `src/crossby/ai_tools/<tool>.py` subclassing `AbstractAITool`:
   ...
   - Implement `capabilities()` returning an `AIToolCapabilities` with at minimum `binary`, `display_name`, `model_flag`, and applicable `supports_*` booleans. Declare `plan_mode=PlanModeCapability(...)` explicitly: use `CLI_ARGUMENT` only when a native selector is verified before the first turn, otherwise use `UNSUPPORTED` with actionable remediation.
   ...
   - If native plan mode is supported, override `plan_mode_args()` with the real
     selector. Never synthesize a slash-command prompt. If plan artifacts live
