...
| Any hook written to `.codex/hooks.json` | `[features].hooks = true` written to `.codex/config.toml` | automatic, defensive | The hooks feature is stable and **on by default** since Codex 0.146.0, so the flag is no longer required. `CodexHooksWriter` keeps the canonical key explicit and removes the deprecated `codex_hooks` alias written by older Crossby versions. A manual-fix note appears only if that file can't be written. |
