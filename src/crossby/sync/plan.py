    ...
    if summary.by_concern:
        concern_summary = ", ".join(
            ...
            for concern, count in sorted(
                summary.by_concern.items(), key=lambda kv: kv[0].value
            )
        )
...
def doctor_readiness(
    plan: PlanSummary, validation: Sequence[ValidationFinding]
) -> Readiness:
    ...
    """
    error_total = plan.error_count + sum(
        1 for f in validation if f.level == "error"
    )
    actionable = error_total + plan.manual_fix_count + sum(
        1 for f in validation if f.level == "warning"
    )
    if error_total > 0:
        return "low"
    if actionable == 0:
...
def render_doctor(report: DoctorReport) -> str:
    ...
    if report.plan.manual_fix_results:
        ...
        for result in report.plan.manual_fix_results:
            ...
            path = result.file_path.as_posix() if result.file_path else "(no path)"
            lines.append(
                f"    - [{tool}] {result.concern.value} {path}: {result.message}"
            )
    ...
    lines.append(
        "  recommended flow: --plan, --dry-run, fix manual items, sync, "
        "--validate-target."
    )
    return "\n".join(lines)
