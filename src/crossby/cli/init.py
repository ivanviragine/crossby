...
def init(
    ...
    force: bool = typer.Option(False, "--force", help="Overwrite an existing .crossby.yml."),
...
def _build_review_fields(state: dict[str, Any], tool_names: list[str]) -> list:  # type: ignore[type-arg]
...
def _pick_token_budget() -> int | None:
    ...
    try:
        ...
        return None
    if value <= 0:
        console.warn(f"Token budget must be positive (got {value}); leaving token budget unset.")
        return None
    return value
...
def _render_init_yaml(answers: dict[str, Any]) -> str:
    ...
    sync_payload = sync_defaults.model_dump(by_alias=True, mode="json", exclude_none=True)
    handoff_payload = handoff_defaults.model_dump(by_alias=True, mode="json", exclude_none=True)
    ...
    def _section(header: str, key: str, payload: dict[str, Any]) -> None:
        parts.append(f"\n# {header}\n")
        if payload:
            parts.append(yaml.safe_dump({key: payload}, sort_keys=False, default_flow_style=False))
        else:
            parts.append(f"{key}: {{}}\n")
