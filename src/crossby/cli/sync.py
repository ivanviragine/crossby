...
def sync(
    ...
    from_tool: str | None = typer.Option(None, "--from", help="Source tool to read configs from."),
    ...
    if strategy is not None and strategy not in valid_strategies:
        console.error(
            f"Unknown --strategy: {strategy!r}. Valid: {', '.join(sorted(valid_strategies))}."
        )
        raise typer.Exit(1)
    ...
    if inspect_modes > 1:
        console.error("--validate-target, --plan, and --doctor are mutually exclusive.")
        raise typer.Exit(1)
    ...
    if data.agents_source and (sync_concern is None or sync_concern == SyncConcern.AGENTS):
        ...
        results += run_sync(
            ...
            dry_run=dry_run,
            force=force,
            installed_tools=agents_targets,
        )
...
def _confirm_sync_defaults(
    *,
    ...
    def _change_concern(current: SyncConcern | None, _state: dict[str, Any]) -> dict[str, Any]:
        _ = current
...
def _run_inspection(
    *,
    ...
    target_tools = [target] if target else [t for t in installed_tools if t != source]
    if not target_tools:
