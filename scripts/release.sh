...
#
# Manual fallback for when the CI pipeline (release.yml / publish.yml) isn't
# available — PyPI's Trusted Publishing (OIDC) only works from within GitHub
# Actions, so a local publish still needs a token.
#
# Usage:
...
VERSION=$(python3 -c "import re; print(re.search(r'__version__\s*=\s*\"(.+?)\"', open('src/crossby/__init__.py').read()).group(1))")
TAG="v${VERSION}"

echo "=== Release crossby ${TAG} ==="
echo ""
