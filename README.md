...
**Revocable sync.** Hooks, permissions and MCP servers are *additive by default but revocable*: syncing `--from A` then `--from B` leaves each target reflecting B, not the union of both. crossby only ever removes an entry it wrote itself — it records what it wrote in a per-machine, gitignored `.crossby/owned.json` ledger and never touches a hand-authored entry (even one that happens to share a name with a source entry). A same-named MCP server you wrote by hand is never deleted; a fresh clone starts with an empty ledger and can only add until it catches up.
