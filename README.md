...
- **A corrupt ledger fails closed.** If `.crossby/owned.json` exists but is unreadable (garbage, wrong shape, a directory, or a symlink — broken *or* pointing at a valid file, since a symlinked ledger is refused rather than followed), both `use` and `clear` — including `--plan` — refuse (exit 1) rather than revert from an empty view, which would leave settings applied with no recovery record. Restore it from backup or revert the applied settings by hand; **never delete it** — a *missing* ledger reads as "crossby owns nothing," which re-opens the exact gap. (A genuinely absent ledger, e.g. a fresh per-machine clone, is fine.)
- **Drift is detected, not clobbered.** `status` compares a per-tool content hash captured at apply time against the current file (normalised, so a semantically-neutral reformat is not flagged; drift is tracked at file granularity, so an unrelated edit to a shared config file such as `.claude/settings.json` can register too). A **symlinked** config is fingerprinted by its *resolved contents*, so editing the link target is drift as well — after upgrading crossby, a config baselined under the old logic may show one-time drift and need a `crossby scene use <name> --force` to refresh the baseline (a plain re-apply hits the drift refusal). `use` and `clear` refuse to revert a scene whose managed files have drifted — pass `--force` to proceed anyway. `--force` **bypasses the refusal**; because reversion is ledger-driven, re-applying then replaces crossby's own entries, but a value you changed by hand to a non-crossby value is left as you set it.
