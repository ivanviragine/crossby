...
class TestMenuDispatch:
    ...
    def _run(self, menu_idx: int, *, extras: list[int] | None = None) -> None:
        ...
        self.monkeypatch.setattr(
            "crossby.ui.prompts.menu", _MenuRecorder([menu_idx])
        )
        if extras is not None:
            select_recorder = _MenuRecorder(extras)
            self.monkeypatch.setattr(
                "crossby.ui.prompts.select", select_recorder
            )
            self.monkeypatch.setattr(
                "crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "x"
            )
        runner.invoke(app, [])
...
class TestPromptHelpers:
    def test_prompt_convert_args_maps_selection_to_string(
        ...
        monkeypatch.setattr(
            "crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "Bash(ls)"
        )
    ...
    def test_prompt_stats_args_with_installed_tool(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        monkeypatch.setattr(
            "crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "/t.txt"
        )
