...
class TestInitNonInteractive:
    def test_writes_file_with_first_installed_tool(
        ...
        result = runner.invoke(app, ["init", "--path", str(tmp_path), "--non-interactive"])
...
class TestInitExistingFileGuard:
    def test_refuses_without_force(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        target = tmp_path / ".crossby.yml"
        existing = "version: 1\nai:\n  default_tool: claude\n"
...
class TestRenderInitYaml:
    ...
    def test_full_answers_round_trip_preserves_every_field(self, tmp_path: Path) -> None:
...
class TestPickTokenBudget:
    def test_non_integer_input_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Invalid input warns the user and leaves the budget unset."""
        from crossby.cli.init import _pick_token_budget

        monkeypatch.setattr("crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "not-an-int")
        assert _pick_token_budget() is None

    def test_blank_input_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from crossby.cli.init import _pick_token_budget

        monkeypatch.setattr("crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "")
        assert _pick_token_budget() is None

    def test_integer_input_returns_value(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from crossby.cli.init import _pick_token_budget

        monkeypatch.setattr("crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "16000")
        assert _pick_token_budget() == 16_000

    def test_zero_input_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """``token_budget: 0`` is invalid — warn and treat as unset."""
        from crossby.cli.init import _pick_token_budget

        monkeypatch.setattr("crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "0")
        assert _pick_token_budget() is None

    def test_negative_input_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Negative ``token_budget`` is invalid — warn and treat as unset."""
        from crossby.cli.init import _pick_token_budget

        monkeypatch.setattr("crossby.ui.prompts.input_prompt", lambda *_a, **_kw: "-100")
        assert _pick_token_budget() is None
