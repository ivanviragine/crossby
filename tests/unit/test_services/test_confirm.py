...
class TestMenuAndChange:
    def test_proceed_exits_without_change(self, tty: None, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_change_fn_updates_value(self, tty: None, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_visible_when_hides_field(self, tty: None, monkeypatch: pytest.MonkeyPatch) -> None:
        recorder = _PromptRecorder([0])
        _install_select(monkeypatch, recorder)
    ...
    def test_custom_menu_label(self, tty: None, monkeypatch: pytest.MonkeyPatch) -> None:
        recorder = _PromptRecorder([0])
        _install_select(monkeypatch, recorder)
    ...
    def test_custom_title_passed_through(self, tty: None, monkeypatch: pytest.MonkeyPatch) -> None:
        recorder = _PromptRecorder([0])
        _install_select(monkeypatch, recorder)
