"""Tests for configure_plan_hooks in all four config modules."""
...
class TestClaudeConfigurePlanHooks:
    """configure_plan_hooks writes to .claude/settings.json → hooks.PreToolUse."""
...
class TestCursorConfigurePlanHooks:
    """configure_plan_hooks writes to .cursor/hooks.json → preToolUse[]."""
    ...
    def test_fresh_install(self, tmp_path: Path) -> None:
        ...
        # plan mode still cannot mutate the filesystem via Cursor's Delete tool.
    ...
    def test_upgrades_existing_matcher(self, tmp_path: Path) -> None:
        ...
        hooks_path = tmp_path / ".cursor" / "hooks.json"
        hooks_path.parent.mkdir(parents=True)
        # Simulate an older install that only covered Write (missing Delete).
        old_config = {
            "version": 1,
            "hooks": {
                "preToolUse": [
                    {"type": "command", "command": str(guard), "matcher": "Write"},
                ]
            },
        }
        ...
        # Widened, never narrowed — coverage grows on re-sync.
        assert matching[0]["matcher"] == "Write|Delete"
...
class TestCopilotConfigurePlanHooks:
    """configure_plan_hooks writes to .github/hooks/hooks.json → hooks.preToolUse[]."""
