...
class TestDetectSkillsSource:
    ...
    def test_finds_gemini_skills_when_no_claude(self, tmp_path: Path) -> None:
        skills = tmp_path / ".gemini" / "skills"
        skills.mkdir(parents=True)
        (skills / "SKILL.md").write_text("skill")
    ...
    def test_finds_agents_skills_when_no_claude_or_gemini(self, tmp_path: Path) -> None:
    ...
    def test_prefers_claude_over_gemini(self, tmp_path: Path) -> None:
