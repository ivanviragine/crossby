...
def _stage_claude_session(fixtures_dir: Path, fake_home: Path, project_root: Path) -> None:
...
def test_handoff_no_launch_skips_target(fixtures_dir: Path, tmp_path: Path, monkeypatch) -> None:
    project_root = tmp_path / "proj"
    project_root.mkdir()
    fake_home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", lambda: fake_home)
...
def _make_raw_subprocess_run(launched: dict[str, object], summary: str):  # type: ignore[no-untyped-def]
    ...
    def _run(cmd, **kwargs):  # type: ignore[no-untyped-def]
        binary = cmd[0] if cmd else ""
        if binary == "claude":
            return subprocess.CompletedProcess(args=cmd, returncode=0, stdout=summary, stderr="")
        launched["cmd"] = list(cmd)
        launched["cwd"] = kwargs.get("cwd")
        return subprocess.CompletedProcess(args=cmd, returncode=0, stdout="", stderr="")
