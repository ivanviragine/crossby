...
from crossby.ai_tools.claude import ClaudeAdapter, _encode_claude_path
...
class TestClaudePreserveSessionData:
    def test_no_source_dir_returns_true(self, tmp_path: Path) -> None:
        """When no working_dir session dir exists, returns True (nothing to copy)."""
        adapter = ClaudeAdapter()
        fake_home = tmp_path / "home"
        (fake_home / ".claude" / "projects").mkdir(parents=True)

        working_dir = tmp_path / "worktrees" / "feat-1-thing"
        main_path = tmp_path / "repo"

        with patch.object(Path, "home", return_value=fake_home):
            result = adapter.preserve_session_data(working_dir, main_path)

        assert result is True

    def test_copies_files_to_main_session_dir(self, tmp_path: Path) -> None:
        """Session files from working_dir are copied to main checkout's project dir."""
        adapter = ClaudeAdapter()
        fake_home = tmp_path / "home"

        working_dir = tmp_path / "worktrees" / "feat-1"
        main_path = tmp_path / "repo"

        # Set up source session dir
        wt_encoded = str(working_dir).replace("/", "-").replace(".", "-")
    ...
    def test_does_not_overwrite_existing_files(self, tmp_path: Path) -> None:
        ...
        working_dir = tmp_path / "worktrees" / "feat-2"
        main_path = tmp_path / "repo"
    ...
    def test_copies_subdirectories(self, tmp_path: Path) -> None:
        ...
        working_dir = tmp_path / "worktrees" / "feat-3"
        main_path = tmp_path / "repo"
    ...
    def test_creates_main_session_dir_if_missing(self, tmp_path: Path) -> None:
        ...
        working_dir = tmp_path / "worktrees" / "feat-4"
        main_path = tmp_path / "repo"
    ...
    def test_copies_with_dotfile_path_segments(self, tmp_path: Path) -> None:
        ...
        working_dir = tmp_path / ".worktrees" / "proj" / "feat-5"
        main_path = tmp_path / "repo"
