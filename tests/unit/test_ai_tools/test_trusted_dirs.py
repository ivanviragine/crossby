...
class TestTrustedDirsLaunchCommand:
    ...
    def test_codex_accept_edits_trusted_dir_no_duplicate_sandbox(self) -> None:
        # accept-edits and trusted dirs both need workspace-write; the composer
        # owns the mode so ``--sandbox workspace-write`` appears exactly once
        # (Codex/clap rejects a duplicate), before the --add-dir.
    ...
    def test_codex_auto_trusted_dir_no_duplicate_sandbox(self) -> None:
        # auto downgrades to accept-edits on Codex; the composer still emits a
        # single sandbox flag.
